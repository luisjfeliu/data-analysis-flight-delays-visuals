...
5. **Geography matters, but state/territory aggregation is coarse:** Origin states and territories differ in both average delay and departure-to-arrival recovery, and the Part II state/territory view should be read as a recovery-gap screen rather than a worst-delay ranking.
...
The Part II deck uses five polished visuals from Part I: delay recovery by departure-delay range (V18), mean delay by cause (V5), day/time delay risk (V15), monthly delay risk (V12), and origin state/territory recovery gaps (V16).
